...
def get_end_of_dr(this_datetime,control_type):
    if control_type == 'peak_shaving':
        end_time = this_datetime.replace(minute=0)+timedelta(hours=1)
    elif control_type == 'net_load':
        if this_datetime.minute<=19:
            end_time = this_datetime.replace(minute=19)
        elif this_datetime.minute<=39:
            end_time = this_datetime.replace(minute=39)
        elif this_datetime.minute<=59:
            end_time = this_datetime.replace(minute=59)
        else:
            end_time = this_datetime
    else:
        end_time = this_datetime
...
def get_signal_from_server():
    ...
    try:
        ...
        r = requests.get('http://yourserverdomain.com/signal')
        current_command_peak = r.json()['peak_shaving_event']
        current_command_net_load = r.json()['net_load_event']
        if current_command_peak:
            control_type = 'peak_shaving'
        elif current_command_net_load:
            control_type = 'net_load'
        current_command = current_command_peak or current_command_net_load
...
def dr_local_control(mfi_hostname,mfi_outlet):
    ...
    if required_fridge_is_off:
        ...
        if lost_connection_to_server:
            ...
            control_type = last_control_type
            time_check = None
            if last_control_type == 'net_load' and last_row_net_load_dict:
                time_check = last_row_net_load_dict['datetime']
            if last_control_type == 'peak_shaving' and last_row_peak_shaving_dict:
                time_check = last_row_peak_shaving_dict['datetime']
            if time_check!=None:
                print 'Last recorded DR at ' + str(time_check)
                dr_end_time = get_end_of_dr(time_check,control_type)
                print 'DR should end at '+ str(dr_end_time)
            if time_check!=None and last_mfi_state == 0 and datetime.utcnow()<=dr_end_time:
                status+= 'Remaining off despite lost connection because '+str(datetime.utcnow())+' is before ' + str(dr_end_time)+'. '
                new_mfi_state = 0
            else:
                updated_limit_counter = last_limit_counter
                new_mfi_state = 1
